...
APP_ID = os.environ.get("APP_ID", "ZY3oyyX94OFpFT75bL5pQw")
APP_SECRET = os.environ.get("APP_SECRET", "zJXtbmKFuwlXLD-H_S2NRLhzwOwnmFbtnVT6PBDFtKg")
# 本应用运行在 5001 端口，回调地址必须与在认证中心注册的完全一致
CALLBACK_URL = os.environ.get("CALLBACK_URL", "http://localhost:5001/oauth/callback")
...
def index():
    ...
    try:
        ...
        return f"<h1>测试应用 test2</h1><p>已登录（获取用户信息失败: {e}）</p><p><a href='/logout'>退出登录</a></p>"
    return (
        "<h1>测试应用 test2</h1>"
        "<p>已登录</p>"
        '<p><a href="/logout">退出登录</a></p>'
